...
def datos():
    try:
        ...
        metas = int (input('Introdusca el numero de objetivos para alcanzar: '))
    except:
    ...
    return [numero_filas,numero_columnas,estructura,eleccion,metas]
...
def objetivo( laberinto, jugador, meta, metas ):
    x, y = [ random.randint( 1, len( laberinto ) - 2 ), random.randint( 1, len( laberinto[ 0 ] ) - 2)]
    if( laberinto[x][y] != "#" and laberinto[x][y] != jugador):
        laberinto[x][y] = meta
        return [x,y]   
    else:
        return objetivo( laberinto, jugador, meta, metas )
             
numero_filas,numero_columnas,estructura,eleccion,metas=datos()
